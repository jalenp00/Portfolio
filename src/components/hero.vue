<template>
    <div class="w-[100vw] flex-row overflow-auto">
        <div v-for="(message, index) in replyArray" :key="index" 
        class="text-white text-[1.25em]"
        v-html="message">
        </div>
        <div class="text-white text-[1.25em]">
            <label id="label" for="input"
            class="align-middle">
            {{ userDir }}</label>
            <textarea id="input" rows="1" cols="50" maxlength="50" v-model="user"
            class="bg-transparent border-none text-white overflow-hidden resize-none focus:outline-none focus:ring-0 align-middle"
